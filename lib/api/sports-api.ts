...
// Generate realistic matches for all sports
...
function getGenericTeamNames(sportSlug: string, country: string): string[] {
  ...
  if (sportSlug === 'tennis' || sportSlug === 'golf' || sportSlug === 'boxing' || sportSlug === 'mma') {
    // Individual sports - return player names
    return [
...
function generateScore(status: string, sportSlug: string): { home: number | null; away: number | null } {
  ...
  // Different score ranges for different sports
...
export function generateMatchesForSport(sportId: number, count: number = 10): Match[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Random time within -2 hours to +48 hours
    const timeOffset = Math.floor(Math.random() * 50 * 60 * 60 * 1000) - (2 * 60 * 60 * 1000);
    const kickoffTime = new Date(now.getTime() + timeOffset);
    ...
    if (timeDiff > 0) {
      status = 'scheduled';
    } else if (timeDiff > -105 * 60 * 1000) { // Within 105 minutes of start
    ...
    matches.push({
      ...
      minute: status === 'live' ? Math.floor(Math.random() * 45) + (Math.random() > 0.5 ? 45 : 0) + 1 : undefined,
      league: {
        id: league.id,
        name: league.name,
...
// Generate matches for all sports
export function generateAllMatches(): Match[] {
  const allMatches: Match[] = [];
  
  // Popular sports get more matches
  ...
  return allMatches.sort((a, b) => new Date(a.kickoffTime).getTime() - new Date(b.kickoffTime).getTime());
}
  ...
  return matches.filter(match => {
    ...
    if (filters.status) {
      ...
      if (!statuses.includes(match.status)) {
        // Special handling for "live" to include halftime
...
export function getTodayMatches(matches: Match[]): Match[] {
  ...
  return matches.filter(m => new Date(m.kickoffTime).toDateString() === today);
}

// Get upcoming matches
export function getUpcomingMatches(matches: Match[]): Match[] {
  const now = new Date();
  return matches.filter(m => m.status === 'scheduled' && new Date(m.kickoffTime) > now);
}
...
// Export helper data
export { ALL_SPORTS, ALL_LEAGUES, BOOKMAKERS };
