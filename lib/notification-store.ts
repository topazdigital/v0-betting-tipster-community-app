...
export async function createNotification(input: Omit<NotificationRow, 'id' | 'createdAt' | 'isRead'>): Promise<NotificationRow> {
  ...
  if (hasDb()) {
    try {
      ...
      const insertId = (r as unknown as { insertId?: number }).insertId;
  ...
  if (hasDb()) {
    try {
      if (ids && ids.length > 0) {
        ...
        count = (r as unknown as { affectedRows?: number }).affectedRows ?? 0;
      } else {
        const r = await query(
...
export async function unsubscribeEmail(token: string): Promise<boolean> {
  ...
  if (hasDb()) {
    try {
      ...
      ok = ((r as unknown as { affectedRows?: number }).affectedRows ?? 0) > 0;
    } catch {}
  }
  for (const [k, v] of stores.emailSubs) {
