...
- **Login Security & Captcha**: `/api/auth/login` and `/api/auth/register` are gated by `lib/rate-limit.ts` (in-process per-IP + per-(email,IP) buckets — 20 logins / 5min, 5 signups / 1h, hard lock after 10 failures) and `lib/captcha.ts` (now async). The provider is resolved in this order: `site_settings.captcha_provider` (set in admin) → env vars (`TURNSTILE_SECRET_KEY` / `RECAPTCHA_SECRET_KEY`) → server-issued math fallback. Admin → Settings → Security now has a "Login Captcha" card to choose the provider and paste Turnstile/reCAPTCHA site + secret keys without touching env vars (keys are persisted to `site_settings` and also mirrored back into `process.env` at request time so existing helpers keep working). The auth modal renders `<CaptchaChallenge>` (always-on for signup, after 3 failed login attempts otherwise). Math challenges live in-memory and are issued/consumed via `/api/captcha/challenge`. Database tables `login_attempts`, `account_lockouts`, `captcha_challenges`, `user_bookmarks` are added to `scripts/setup-database-consolidated.sql`.
...
- **`/api/bookmarks` endpoint** (`app/api/bookmarks/route.ts`): GET/POST/DELETE backed by the `user_bookmarks(user_id, entity_type, entity_id, created_at)` table created in `setup-database-consolidated.sql`. Auth is resolved via `getCurrentUser()` from `lib/auth.ts` (cookie `betcheza_auth`, JWT payload `{userId, email, role}`). When the database is unreachable the endpoint falls back to a per-process in-memory map keyed by `${userId}:${entityType}:${entityId}` so the bookmark icon on the match details page (line ~1015 in `app/(main)/matches/[id]/page.tsx`) keeps working. Returns 401 when unauthenticated; localStorage remains the offline source of truth on the client.

- **Match details — compact, oddspedia-style hero** (`app/(main)/matches/[id]/page.tsx`): The hero card was tightened (rounded-xl, `mb-3`, `px-3 py-3 md:px-5 md:py-4`, scores at `text-4xl md:text-5xl` instead of `text-5xl md:text-7xl`, medium logos, smaller form badges and meta text). Outer page padding dropped to `py-2 md:py-3` with a `mb-2` back link, and the right rail keeps its existing `sticky top-4` bet-slip layout so the page reads more like Oddspedia/Sofascore at a glance.

- **Wider match coverage** (`lib/api/unified-sports-api.ts`, `app/api/matches/route.ts`): `ESPN_LEAGUES` now also pulls `usa.usl.1`, `usa.usl.cup`, `eng.w.1` (WSL), `fra.w.1`, `esp.w.1`, `ger.w.1`, `ita.w.1` and `uefa.wchampions` so the daily slate matches what ESPN actually shows (UEFA CL, NWSL, WSL, Primeira Liga, Saudi, Libertadores, CONCACAF, USL Cup, USL Champ, etc). The default `/api/matches` filter (no `status=` param) keeps **today's finished matches** in the response — finished games on other days are still excluded and live on `/results` — so the homepage and `/matches` "Today" tab show the full daily slate (final scores + still-to-play) without bouncing between tabs. Today's count went from ~36 to 80+ matches.
