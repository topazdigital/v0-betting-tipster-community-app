...
- **Responsive lineup pitch with bucketed positions** (`app/(main)/matches/[id]/page.tsx > FormationPitch`): horizontal landscape pitch on `md+` (home left half, away right half), vertical portrait pitch on mobile (home top, away bottom, attacking lines meet at the centre line). Players are classified into role buckets (GK/DEF/DM/CM/AM/WING/FW) by their position abbreviation and allocated to the correct formation row with neighbour fallbacks — so 4-2-3-1 puts CDMs in the DM line and CAMs in the AM line, 4-3-3 puts wingers in the front line, etc., instead of the previous naïve back-to-front slicing.
...
- **Pre-deployment polish batch (Apr 26, 2026 — late)**:
  ...
  - **Featured tips with admin Tips browser + Live Now injection** — `lib/featured-store.ts` (DB-backed via `featured_config` table, in-memory fallback) tracks `pinnedMatchIds` AND `hiddenMatchIds`. Public `app/api/featured/route.ts` resolves pinned first then auto-fills today's upcoming matches that pass criteria (`minConfidence`, `minOdds`/`maxOdds`, `topTipsterOnly`, `sport`, `limit`), skipping any in `hiddenMatchIds`. Admin endpoint `app/api/admin/featured/tips/route.ts` returns every pinned match plus today's upcoming matches each annotated with `qualifies`/`skipReasons`/`pinned`/`hidden` flags so admins can manage everything inline. Admin page `app/admin/featured/page.tsx` is now tabbed: a **Tips** tab lists pinned + today's auto-generated picks with one-click Pin/Unpin/Hide/Unhide buttons (filter chips: all/qualifying/skipped/hidden, opens match in new tab) and a **Settings** tab for the auto-fill rules. Homepage uses `useFavoritedTips()` to share one fetch — when **0 live matches** the standalone `<FavoritedTipsPanel>` renders below Live Now; when **1-3 live matches**, the same featured tips are injected into the `LiveMarquee` row as `<FavoritedTipMarqueeCard>` (same w-80 width and stretched height as MatchCardNew, clearly labelled "Featured Tip" with an amber border).
